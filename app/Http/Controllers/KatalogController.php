...
class KatalogController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $daftarBuku = Book::query()
            ...
            ->when($kueri !== '', fn (Builder $q) => $q->where(function (Builder $sub) use ($kueri) {
                $sub->where('title', 'like', "%{$kueri}%")
                    ->orWhere('author', 'like', "%{$kueri}%")
                    ->orWhere('description', 'like', "%{$kueri}%");
            }))
            ->when($kategoriId, fn (Builder $q) => $q->where('category_id', $kategoriId))
