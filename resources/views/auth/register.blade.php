<x-guest-layout>
    <div class="mb-6">
        <h1 class="font-display text-2xl font-semibold text-netral-100">Buat Akun Mahasiswa</h1>
        <p class="mt-1 text-sm text-netral-300">
    ...
    <form method="POST" action="{{ route('register') }}"
        ...
        <div>
            ...
            <p class="mt-1 text-xs text-netral-300">
        ...
        <div class="mt-6 flex items-center justify-between">
            <a href="{{ route('login') }}" class="text-sm text-netral-300 underline hover:text-netral-100">
                Sudah punya akun?
            </a>
        ...
        <div x-show="konfirmasi" x-cloak
             class="fixed inset-0 z-50 flex items-center justify-center bg-arang-deepest/50 px-4">
            <div class="w-full max-w-md border border-arang-base bg-arang-deep p-6 shadow-lg" @click.outside="konfirmasi = false">
                <h2 class="font-display text-lg font-semibold text-netral-100">Periksa sekali lagi</h2>

                <p class="mt-2 text-sm text-netral-300">
                ...
                <div class="mt-6 flex justify-end gap-3">
                    <button type="button" @click="konfirmasi = false"
                            class="rounded-sm border border-arang-base px-4 py-2 text-sm font-medium text-netral-200 transition-colors hover:bg-arang-base">
                        Periksa Ulang
                    </button>

                    <button type="button" @click="$el.closest('form').submit()"
                            class="rounded-sm bg-sienna px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-sienna-dark">
                        Ya, Daftarkan Saya
                    </button>
                </div>
            </div>
