...
<div x-data="{ mode: '{{ old('access_mode', $buku->access_mode ?? 'readonly') }}' }">
    ...
    <div class="mt-4">
        ...
        <textarea id="description" name="description" rows="3" maxlength="2000"
                  class="mt-1 w-full rounded-sm border-kabut-300 text-kabut-900 focus:border-jingga-500 focus:ring-1 focus:ring-jingga-500">{{ old('description', $buku->description ?? '') }}</textarea>
        <x-input-error :messages="$errors->get('description')" class="mt-2" />
    </div>

    {{-- Lingkup --}}
    <fieldset class="mt-6">
        <legend class="text-sm font-semibold text-kabut-700">Lingkup Buku</legend>
        ...
        <label class="mt-3 flex cursor-pointer gap-3 border border-kabut-200 p-4 hover:bg-kabut-50">
            ...
            <span>
                ...
                <span class="block text-xs text-kabut-500">Hanya mahasiswa program studi Anda yang dapat melihatnya.</span>
            </span>
        </label>
        ...
        <label class="mt-2 flex cursor-pointer gap-3 border border-kabut-200 p-4 hover:bg-kabut-50">
            ...
            <span>
                ...
                <span class="block text-xs text-kabut-500">Dapat dilihat mahasiswa dari seluruh program studi.</span>
            </span>
        </label>
    ...
    {{-- Berkas --}}
    <div class="mt-6 border-t border-kabut-200 pt-6">
        ...
        <input id="berkas" name="berkas" type="file" accept="application/pdf" @required(! $buku)
               class="mt-1 block w-full text-sm text-kabut-700 file:mr-3 file:rounded-sm file:border-0 file:bg-sepia-800 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-kabut-50 hover:file:bg-sepia-900">
        <p class="mt-1 text-xs text-kabut-500">Format PDF, maksimal 30 MB.</p>
        @if ($buku)
            <p class="mt-1 text-xs text-kabut-500">Berkas saat ini: {{ $buku->ukuranMb() }} MB. Kosongkan bila tidak ingin mengganti.</p>
        @endif
    ...
    <div class="mt-4">
        ...
        <input id="sampul" name="sampul" type="file" accept="image/*"
               class="mt-1 block w-full text-sm text-kabut-700 file:mr-3 file:rounded-sm file:border-0 file:bg-kabut-200 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-kabut-700 hover:file:bg-kabut-300">
        <x-input-error :messages="$errors->get('sampul')" class="mt-2" />
    </div>

    {{-- Aturan unduh --}}
    <fieldset class="mt-6 border-t border-kabut-200 pt-6">
        <legend class="text-sm font-semibold text-kabut-700">Aturan Unduh</legend>

        <label class="mt-3 flex cursor-pointer gap-3 border border-kabut-200 p-4 hover:bg-kabut-50">
            <input type="radio" name="access_mode" value="full" x-model="mode" class="mt-0.5 text-jingga-600 focus:ring-jingga-500" required>
            <span>
                <span class="block text-sm font-medium text-kabut-900">Unduh penuh</span>
                <span class="block text-xs text-kabut-500">Mahasiswa boleh mengunduh seluruh isi buku.</span>
            </span>
        </label>
        ...
        <label class="mt-2 flex cursor-pointer gap-3 border border-kabut-200 p-4 hover:bg-kabut-50">
            ...
            <span>
                <span class="block text-sm font-medium text-kabut-900">Unduh sebagian</span>
                <span class="block text-xs text-kabut-500">Hanya rentang halaman tertentu yang boleh diunduh.</span>
            </span>
        </label>
        ...
        <label class="mt-2 flex cursor-pointer gap-3 border border-kabut-200 p-4 hover:bg-kabut-50">
            ...
            <span>
                <span class="block text-sm font-medium text-kabut-900">Baca saja</span>
                <span class="block text-xs text-kabut-500">Buku hanya dapat dibaca di peramban, tidak dapat diunduh.</span>
            </span>
        </label>
        ...
        <div x-show="mode === 'partial'" x-cloak class="mt-3 grid gap-4 border border-jingga-200 bg-jingga-50 p-4 sm:grid-cols-2">
            <div>
    ...
    {{-- Opsi tambahan --}}
    <div class="mt-6 space-y-3 border-t border-kabut-200 pt-6">
        <label class="flex items-start gap-3">
            <input type="hidden" name="watermark_enabled" value="0">
            <input type="checkbox" name="watermark_enabled" value="1"
                   class="mt-0.5 rounded-sm border-kabut-300 text-jingga-600 focus:ring-jingga-500"
                   @checked(old('watermark_enabled', $buku->watermark_enabled ?? true))>
            <span>
                <span class="block text-sm font-medium text-kabut-900">Aktifkan watermark</span>
                <span class="block text-xs text-kabut-500">Nama dan email mahasiswa dicantumkan pada berkas unduhan.</span>
            </span>
        </label>
        ...
        <label class="flex items-start gap-3">
            ...
            <span>
                <span class="block text-sm font-medium text-kabut-900">Terbitkan sekarang</span>
                <span class="block text-xs text-kabut-500">Nonaktifkan bila ingin menyimpannya sebagai draf.</span>
            </span>
        </label>
    </div>
</div>