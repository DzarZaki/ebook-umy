...
<div x-data="{ mode: '{{ old('access_mode', $buku->access_mode ?? 'readonly') }}' }">
    ...
    <div class="mt-4">
        ...
        <textarea id="description" name="description" rows="3" maxlength="2000"
                  class="mt-1 w-full rounded border-netral-200 dark:border-arang-600 bg-white dark:bg-arang-700/50 text-netral-900 dark:text-netral-100 focus:border-jingga-600 dark:focus:border-jingga-500 focus:ring-1 focus:ring-jingga-500 shadow-sm">{{ old('description', $buku->description ?? '') }}</textarea>
        <x-input-error :messages="$errors->get('description')" class="mt-2" />
    </div>

    {{-- Lingkup --}}
    <fieldset class="mt-6">
        <legend class="text-sm font-semibold text-netral-700 dark:text-netral-300">Lingkup Buku</legend>
        ...
        <label class="mt-3 flex cursor-pointer gap-3 border border-netral-200 dark:border-arang-600 rounded-lg p-4 bg-white/60 dark:bg-arang-700/30 hover:bg-netral-50 dark:hover:bg-arang-700/60 shadow-sm transition-colors">
            ...
            <span>
                ...
                <span class="block text-xs text-netral-500 dark:text-netral-400">Hanya mahasiswa program studi Anda yang dapat melihatnya.</span>
            </span>
        </label>
        ...
        <label class="mt-2 flex cursor-pointer gap-3 border border-netral-200 dark:border-arang-600 rounded-lg p-4 bg-white/60 dark:bg-arang-700/30 hover:bg-netral-50 dark:hover:bg-arang-700/60 shadow-sm transition-colors">
            ...
            <span>
                ...
                <span class="block text-xs text-netral-500 dark:text-netral-400">Dapat dilihat mahasiswa dari seluruh program studi.</span>
            </span>
        </label>
    ...
    <div class="mt-4">
        ...
        <select id="category_id" name="category_id"
                class="mt-1 block w-full rounded border-netral-200 dark:border-arang-600 bg-white dark:bg-arang-700/50 text-netral-900 dark:text-netral-100 focus:border-jingga-600 dark:focus:border-jingga-500 focus:ring-jingga-500 shadow-sm">
    ...
    <div class="mt-6 border-t border-netral-200 dark:border-arang-600 pt-6">
        ...
        <input id="berkas" name="berkas" type="file" accept="application/pdf" @required(! $buku)
               class="mt-1 block w-full text-sm text-netral-600 dark:text-netral-300 file:mr-3 file:rounded file:border-0 file:bg-netral-100 dark:file:bg-arang-700 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-netral-700 dark:file:text-netral-100 hover:file:bg-netral-200 dark:hover:file:bg-arang-600 cursor-pointer">
        <p class="mt-1 text-xs text-netral-500 dark:text-netral-400">Format PDF, maksimal 30 MB.</p>
        @if ($buku)
            <p class="mt-1 text-xs text-netral-500 dark:text-netral-400">Berkas saat ini: {{ $buku->ukuranMb() }} MB. Kosongkan bila tidak ingin mengganti.</p>
        @endif
    ...
    {{-- Aturan unduh --}}
    <fieldset class="mt-6 border-t border-netral-200 dark:border-arang-600 pt-6">
        <legend class="text-sm font-semibold text-netral-700 dark:text-netral-300">Aturan Unduh</legend>

        <label class="mt-3 flex cursor-pointer gap-3 border border-netral-200 dark:border-arang-600 rounded-lg p-4 bg-white/60 dark:bg-arang-700/30 hover:bg-netral-50 dark:hover:bg-arang-700/60 shadow-sm transition-colors">
            <input type="radio" name="access_mode" value="full" x-model="mode" class="mt-0.5 text-jingga-600 focus:ring-jingga-500" required>
            <span>
                <span class="block text-sm font-medium text-netral-900 dark:text-netral-100">Unduh penuh</span>
                <span class="block text-xs text-netral-500 dark:text-netral-400">Mahasiswa boleh mengunduh seluruh isi buku.</span>
            </span>
        </label>
        ...
        <label class="mt-2 flex cursor-pointer gap-3 border border-netral-200 dark:border-arang-600 rounded-lg p-4 bg-white/60 dark:bg-arang-700/30 hover:bg-netral-50 dark:hover:bg-arang-700/60 shadow-sm transition-colors">
            ...
            <span>
                <span class="block text-sm font-medium text-netral-900 dark:text-netral-100">Unduh sebagian</span>
                <span class="block text-xs text-netral-500 dark:text-netral-400">Hanya rentang halaman tertentu yang boleh diunduh.</span>
            </span>
        </label>
        ...
        <label class="mt-2 flex cursor-pointer gap-3 border border-netral-200 dark:border-arang-600 rounded-lg p-4 bg-white/60 dark:bg-arang-700/30 hover:bg-netral-50 dark:hover:bg-arang-700/60 shadow-sm transition-colors">
            ...
            <span>
                <span class="block text-sm font-medium text-netral-900 dark:text-netral-100">Baca saja</span>
                <span class="block text-xs text-netral-500 dark:text-netral-400">Buku hanya dapat dibaca langsung di situs, tidak dapat diunduh.</span>
            </span>
        </label>
        ...
        <div x-show="mode === 'partial'" x-cloak class="mt-3 grid gap-4 border border-jingga-300 dark:border-jingga-700/50 bg-jingga-50/80 dark:bg-jingga-900/30 p-4 rounded-lg sm:grid-cols-2">
            <div>
    ...
    {{-- Opsi tambahan --}}
    <div class="mt-6 space-y-3 border-t border-netral-200 dark:border-arang-600 pt-6">
        <label class="flex items-start gap-3 cursor-pointer">
            <input type="hidden" name="watermark_enabled" value="0">
            <input type="checkbox" name="watermark_enabled" value="1"
                   class="mt-0.5 rounded border-netral-300 dark:border-arang-500 text-jingga-600 focus:ring-jingga-500"
                   @checked(old('watermark_enabled', $buku->watermark_enabled ?? true))>
            <span>
                <span class="block text-sm font-medium text-netral-900 dark:text-netral-100">Aktifkan watermark</span>
                <span class="block text-xs text-netral-500 dark:text-netral-400">Nama dan email mahasiswa dicantumkan pada berkas unduhan.</span>
            </span>
        </label>
        ...
        <label class="flex items-start gap-3 cursor-pointer">
            ...
            <span>
                <span class="block text-sm font-medium text-netral-900 dark:text-netral-100">Terbitkan sekarang</span>
                <span class="block text-xs text-netral-500 dark:text-netral-400">Nonaktifkan bila ingin menyimpannya sebagai draf.</span>
            </span>
        </label>
    </div>
</div>