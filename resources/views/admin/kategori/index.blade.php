<x-app-layout>
    <x-slot name="header">
        <h2 class="font-display text-xl font-semibold leading-tight text-kabut-800">Kategori</h2>
    </x-slot>

    <div class="py-10">
        <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            <x-alert-status />

            <div class="mb-4 flex justify-end">
                <a href="{{ route('admin.kategori.create') }}"
                   class="rounded-sm bg-jingga-600 px-4 py-2 text-sm font-semibold text-white hover:bg-jingga-700">
                    Tambah Kategori
                </a>
            </div>

            <div class="overflow-hidden border border-kabut-200 bg-white">
                <table class="min-w-full divide-y divide-kabut-200 text-sm">
                    <thead class="bg-kabut-100 text-left text-kabut-600">
                        <tr>
                            <th class="px-4 py-3 font-medium">Nama Kategori</th>
                            <th class="px-4 py-3 font-medium">Lingkup</th>
                            <th class="px-4 py-3 font-medium">Jumlah Buku</th>
                            <th class="px-4 py-3 text-right font-medium">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-kabut-100">
                        @forelse ($daftarKategori as $kategori)
                            <tr>
                                <td class="px-4 py-3 font-medium text-kabut-900">{{ $kategori->name }}</td>
                                <td class="px-4 py-3">
                                    @if ($kategori->isUmum())
                                        <span class="rounded-sm bg-sepia-100 px-2 py-1 text-xs font-medium text-sepia-800">Umum</span>
                                    @else
                                        <span class="rounded-sm bg-jingga-50 px-2 py-1 text-xs font-medium text-jingga-800">{{ $kategori->prodi->name }}</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-kabut-700">{{ $kategori->books_count }}</td>
                                <td class="px-4 py-3">
                                    @if ($kategori->bolehDikelolaOleh(auth()->user()))
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="{{ route('admin.kategori.edit', $kategori) }}"
                                               class="rounded-sm border border-kabut-300 px-3 py-1.5 text-sm font-medium text-kabut-700 hover:bg-kabut-100">
                                                Ubah
                                            </a>
                                            <x-tombol-hapus
                                                :action="route('admin.kategori.destroy', $kategori)"
                                                judul="Hapus Kategori"
                                                :pesan="'Kategori &quot;'.$kategori->name.'&quot; akan dihapus permanen.'" />
                                        </div>
                                    @else
                                        <p class="text-right text-xs text-kabut-400">Dikelola dosen lain</p>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="4" class="px-4 py-8 text-center text-kabut-500">Belum ada kategori.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>

            <div class="mt-4">{{ $daftarKategori->links() }}</div>
        </div>
    </div>
</x-app-layout>